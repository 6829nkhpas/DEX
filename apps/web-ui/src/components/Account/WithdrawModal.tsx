// ---------------------------------------------------------------------------
// WithdrawModal — stub withdrawal flow (no contract integration yet)
// ---------------------------------------------------------------------------
...
export const WithdrawModal: React.FC<WithdrawModalProps> = ({ onClose }) => {
  ...
  const handleSubmit = useCallback(
    (e: React.FormEvent) => {
      ...
      console.log(`[WithdrawModal] Withdraw ${amount} ${asset}`);
      setSubmitted(true);
  ...
  return (
    ...
    >
      ...
      >
        <h3 className="text-sm font-bold text-white mb-4">Withdraw</h3>

        {submitted ? (
          <div className="text-yellow-400 text-sm text-center py-4">
            ✓ Withdrawal of {amount} {asset} submitted (stub)
          </div>
        ) : (
          <form className="flex flex-col gap-3" onSubmit={handleSubmit}>
            ...
            <div className="flex gap-2 mt-2">
              <button
                type="submit"
                className="flex-1 py-2 text-sm font-medium bg-red-700 hover:bg-red-600 text-white rounded transition-colors"
              >
                Withdraw
              </button>
              <button
