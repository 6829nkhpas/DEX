...
pub fn validate_balance_conservation(
    ...
    let expected_total = expected_total_deposits - expected_total_withdrawals - total_fees_collected;
    
    if system_total != expected_total {
        return Err("Balance conservation violation: System total != Deposits - Withdrawals - Fees");
    }
...
mod tests {
    ...
    fn test_conservation_invariant() {
        ...
        assert_eq!(
            validate_balance_conservation(&[b1, b2], Decimal::from(30), Decimal::from(3), Decimal::from(2)),
            Ok(())
        );
    }
}
