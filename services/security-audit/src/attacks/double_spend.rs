...
pub fn handle_order_placement(store: Arc<AccountStore>, account_id: AccountId, amount: Decimal) -> Result<(), StoreError> {
    let mut current_account = store.get(&account_id).ok_or(StoreError::NotFound)?;
    
    // Check balance
    let balance = current_account.get_balance_mut("USDC").ok_or(StoreError::NotFound)?;
...
pub fn handle_withdrawal(store: Arc<AccountStore>, account_id: AccountId, amount: Decimal) -> Result<(), StoreError> {
    let mut current_account = store.get(&account_id).ok_or(StoreError::NotFound)?;
...
mod tests {
    ...
    fn test_double_spend_mitigation() {
        ...
        // Start with 100 USDC
        account.set_balance(Balance::new("USDC", Decimal::from(100)), 1708123456789000000);
        ...
        mutated_for_order.get_balance_mut("USDC").unwrap().lock(amount_to_spend);
        ...
        assert_eq!(store.update(mutated_for_withdraw), Err(StoreError::VersionConflict));
