...
# ──────────────────────────────────────────────────────
# Usage:
#   ./deploy-staging.sh                  # deploy latest
#   ./deploy-staging.sh v1.2.3           # deploy specific version
#   ./deploy-staging.sh --dry-run        # validate only
# ──────────────────────────────────────────────────────
set -euo pipefail

readonly SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
readonly INFRA_DIR="$(cd "$SCRIPT_DIR/.." && pwd)"
readonly NAMESPACE="dex-exchange-staging"
...
log_warn()  { echo -e "${YELLOW}[deploy]${NC} $*" >&2; }
...
preflight() {
  ...
  log_info "Cluster connected, namespace $NAMESPACE exists"
}
...
# ─── Main ────────────────────────────────────────────
main() {
  log_step "DEX Exchange — Staging Deployment"
